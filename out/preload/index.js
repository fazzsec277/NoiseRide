...
const api = {
  ...
  lastPlayed: {
    setKey: (acc) => electron.ipcRenderer.invoke("lastPlayed:setKey", acc),
    onTrigger: (cb) => {
      const h = () => cb();
      electron.ipcRenderer.on("lastPlayed:trigger", h);
      return () => electron.ipcRenderer.off("lastPlayed:trigger", h);
    }
  },
  dialog: {
